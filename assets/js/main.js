	...
	/******************************/
		...
		// Settings
			settings: {
				...
				// Popup overlay color. 
                //Всплывающее наложения цвета
					popupOverlayColor: '#1a1f2c',

				// Popup overlay opacity. 
                //Всплывающее окно непрозрачность
					popupOverlayOpacity: 0.75,

				// Mobile only.

					// If true, mobile mode will get some automatic styling. 
                    //Если это правда, мобильный режим будет получить некоторые автоматические для укладки
						autoStyleMobile: true,

				// Desktop only.

					// If true, reel will be vertically centered.
                    //Если это правда, катушка будет вертикально по центру
						centerVertically: true,

					// Delay (in ms) before showing the reel.
                    // Задержка (в мс) перед выводом катушки
						introDelay: 600,

					// Speed (in ms) at which to fade in reel.
                    //Скорости (в мс), при которой исчезают в барабане
						introSpeed: 750,

					// Height (in px) of items.
                    // Высота (в пикселях) элементов
						itemHeight: 230,

					// Default width (in px) of width.
                    // По умолчанию ширина (в пикселах) ширины
						itemWidth: 300,

					// Margin (in px) to preserve at the bottom of the viewport.
                    // Маржа (в пикселях), чтобы сохранить в нижней части области просмотра.
						marginBottom: 0,

					// Margin (in px) to preserve at the top of the viewport.
                    // Маржа (в пикселях), чтобы сохранить в верхней части области просмотра
						marginTop: 0,

					// Nudge the reel by this value (in px) after it's been vertically centered.
                    // Подтолкнуть барабан на эту величину (в пикселях) после ее вертикально по центру   (можно поместить второе меню)
						verticalNudge: -30,
                    ...
                    // Если это правда, барабан прокрутки сбросит на обновление страницы
						resetScroll: true,

					// Scales the scroll delta (1 = normal, 2 = double, 0.5 = half, etc.).
                    // Весы свиток Дельта (1 = нормальный, 2 = двойной, 0.5 = половина и т. д)
						scrollFactor: 1,

					// Scroll amount when using keys.
                    // Количество прокрутки при использовании ключей
						scrollKeyAmount: 50,

					// Determines where scrollwheel events should be captured ('window' or 'reel').
                    // Определяет, где колесо прокрутки события должны быть захвачены ('окно' или 'катушка')
						scrollWheelTarget: 'window',

					// Scroll amount when using scroll zones.
                    // Количество прокрутки при использовании прокрутки зон
						scrollZoneAmount: 10,

					// Time (in ms) to wait between scrolls when the cursor is in a scroll zone.
                    // Время (в мс) ожидания между свитки, когда курсор находится в зоне прокрутки.
						scrollZoneDelay: 20,

					// Width of scroll zones.
                    // Ширина зоны прокрутки
						scrollZoneWidth: 50,

					// Delay (in ms) before showing thumbnails.
                    // Задержки (в мс) перед показом миниатюр.
						thumbDelay: 1200,

					// Spread (in ms) to randomly stagger thumbnails (0 = don't stagger).
                    // Распространения (в мс) для произвольно шатаются миниатюр (0 = не шатаются). 
						thumbDelaySpread: 1500,

					// Speed (in ms) at which to fade in thumbnails.
                    // Скорости (в мс), при которой исчезают в миниатюры.
						thumbSpeed: 750,

					// If true, page will blur slightly when the popup is displayed.
                    // Если True, страница будет размыть слегка при открытии всплывающего окна.
						useBlurFilter: true,

					// If true, the left/right arrow keys will scroll the reel.
                    // Если True, слева/правую клавиши со стрелками будет прокручивать барабан.
						useScrollKeys: true,

					// If true, moving the cursor to the leftmost/rightmost edges of the reel will scroll it.
                    // Если True, то перемещение курсора левый/правый края катушки будут прокручивать его.
						useScrollZones: true
			...
			initDesktop: function() {
					...
					_.objects.window._parallelism_update = function() {
                        ...
                        // Изменить размер элементов.
							...
							_.objects.items.each(function(i) {
                                ...
                                // Определить конец строки условия.

									// Current item would push us past the row width.
                                    // Текущий элемент будет толкать нас мимо ширине ряда.
										if (w + iw >= rowWidth) {
					...
					// Scroll Zones.
						if (!_.isTouch && _.settings.useScrollZones) {
							...
							$SZ
								...
								.css('height', 100)
								.css('z-index', 100)
								.css('background', 'rgba(22,0,49,0.2)') // Required due to a weird IE bug (affects <=10)
								.css('top', 0);
				...
				// Poptrox.
					_.objects.reel.poptrox({
						...
						usePopupCloser: true,   // крестик вверху фотографии
